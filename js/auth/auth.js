 ...
 * ============================================================
 * 模块页用法（页面需先引入 supabase.min.js）：
 *
 *   MVAuth.init({ module:'finance', minRole:'viewer', base:'../' }).then(ctx => {
 *     // ctx.client / ctx.email / ctx.role（该模块角色 admin|editor|viewer）
 *   });
 *
 * 平台入口用法（只要求登录，返回全部权限，由页面按权限渲染模块卡）：
 *
 *   MVAuth.initPortal({ base:'' }).then(ctx => {
 *     // ctx.perms = [{module, role}, ...]；ctx.can('finance') -> 'admin'|'editor'|'viewer'|null
 *   });
 *
 * 权限模型：app_permissions (email / module / role)
...
(function (global) {
  ...
  function showLoginOverlay(opts) {
    ...
    wrap.innerHTML = `
      <div class="mvauth-card">
        ...
        <p class="mvauth-sub">${opts.title || 'Management Platform'}</p>
        ...
        <div class="mvauth-hint">仅限授权人员访问 · 账号由管理员开通<br>MamaVege · 妈子素 管理层平台</div>
      </div>`;
  ...
  function showDeniedOverlay(opts, email, extraText) {
    ensureStyles();
    removeOverlay();
    const wrap = document.createElement('div');
    wrap.className = 'mvauth-overlay';
    ...
    wrap.innerHTML = `
      <div class="mvauth-card">
        ...
        <div class="mvauth-denied-text">
          ${extraText || ('账号 <b>' + email + '</b> 没有「' + (opts.moduleName || opts.module) + '」模块的访问权限。<br>如需开通，请联系管理员。')}
        </div>
  ...
  async function getSessionOrLogin(opts) {
    const supa = getClient();
    let { data: { session } } = await supa.auth.getSession();
    if (!session) session = await waitForSignIn(opts);
    return session;
  }

  // 读该用户全部权限行（RLS 只允许读自己的行）
  ...
  function roleFor(perms, module) {
    let best = null;
    perms.forEach(r => {
      if (r.module === module || r.module === 'all') {
  ...
  async function init(opts) {
    ...
    const session = await getSessionOrLogin(opts);
    const email = (session.user.email || '').toLowerCase();

    let perms = [];
    try { perms = await fetchPerms(); }
    catch (e) { console.error('MVAuth: 权限查询失败', e); }

    const role = roleFor(perms, opts.module);
    ...
    removeOverlay();
    return { client: getClient(), user: session.user, email, role, perms };
  }

  // 平台入口守卫：只要求登录 + 至少有一条权限
  async function initPortal(opts) {
    opts = opts || {};
    const session = await getSessionOrLogin(opts);
    const email = (session.user.email || '').toLowerCase();
    ...
    if (!perms.length) {
      showDeniedOverlay(opts, email, '账号 <b>' + email + '</b> 尚未开通任何模块权限。<br>如需开通，请联系管理员。');
      return new Promise(() => {});
    }

    removeOverlay();
    return {
      client: getClient(), user: session.user, email, perms,
      can: module => roleFor(perms, module)
    };
  }
  ...
  global.MVAuth = { init, initPortal, logout, getClient };
})(window);
